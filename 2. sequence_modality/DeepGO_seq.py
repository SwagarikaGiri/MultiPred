...
"""Whenever GPU is not present """

config = tf.ConfigProto(intra_op_parallelism_threads=5, inter_op_parallelism_threads=5, allow_soft_placement=True, log_device_placement=True)
sess = tf.Session(config=config)
K.set_session(sess)
...
def load_data():
    df = pd.read_pickle(DATA_ROOT + 'structuretrain' + '-' + FUNCTION + '.pkl')

    test_df = pd.read_pickle(DATA_ROOT + 'structuretest' + '-' + FUNCTION + '.pkl')
    n = len(df)
...
def model(batch_size=128, nb_epoch=5, is_train=True):
    # set parameters:
    ...
    print preds
    print len(preds)
    print "Its the length of preds"
    print test_labels
    print len(test_labels)
    print "Its the length of test_labels"
    print test_gos
    print len(test_gos)
    print "Its the length of test_gos"
